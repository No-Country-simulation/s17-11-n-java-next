...
const HeaderProfile: React.FC<HeaderProfileProps> = ({
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 ">
      <Card className="rounded-3xl bg-[#F1F1F1]">
        <CardHeader>
          <div className="flex items-center space-x-4">
            ...
            <div>
              <CardTitle className="text-[28px]">{user.name}</CardTitle>
              <div className="flex">
                {[...Array(5)].map((_, i) => (
                  <Star
                    key={i}
          ...
          >
            ...
            <div>
              <CardHeader className="py-3">
                <CardTitle className="text-sm font-medium">Producto</CardTitle>
                <p className="text-xs text-gray-500">Cambio por producto</p>
              </CardHeader>
              <CardContent>
      ...
      {authUser && (
        <Button className="w-full md:w-3/5 mx-auto col-start-[2] bg-[#F6B40E] font-bold h-[50px] hover:bg-[#F7C036]">
          Publicar anuncio
        </Button>
      )}
    </div>
